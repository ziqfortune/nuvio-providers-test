...
// Fungsi utama yang Nuvio akan panggil
async function getStreams(args) {
  const { type, id } = args;
  const tmdbId = id.replace(/^(tmdb:|tt)/, "");
  ...
  try {
    ...
    const pageRes = await axios.get(postLinks[0], {
      headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36" }
    });
    ...
    $$("#playeroptionsul li, .dooplay_player_option, [data-post][data-nume]").each((i, el) => {
      ...
      const typeVal = $$(el).attr("data-type") || "movie";
      ...
      if (postId && nume) {
        serverItems.push({ postId, type: typeVal, nume, serverTitle: formattedTitle });
      }
    });
